...
def main():
    ...
    with (pd.ExcelWriter(out_path, engine="xlsxwriter") if _export_xlsx else contextlib.nullcontext()) as writer:
        ...
        ADJUST_MAP = {
            ...
            "vml":    {"調整金額": ["adjustment_amount", "調整金額"],
                       "調整一級": {"cols": ["adjust_lv1"], "flags": [
                           ...
                           # 2026-06 vml_2025.xlsx 實際 flag 欄（audit_adjust_dims: 25 bucket 1,924 行漏 lv1）
                           "未獲認可的會展活動支出", "其他日常營運支出調整",
                           "超出可計入範圍的內部資源支出"]},
                       "調整二級": ["adjust_lv2"]},
            ...
            "mgm":    {"調整一級": ["Adj_lv1", "adjust_lv1", "調整項目名稱", "調整事項"],   # 25/24/23 檔各自欄名
                       "調整二級": ["Adj_lv2", "adjust_lv2", "調整事項備註", "調整補充"]},  # 23 lv1=調整事項 lv2=調整補充
        ...
        # base = raw cols['amount']（step5 階段已 populated）。注意：wynn Net-off comp 行嘅 base 仲攞住
        # gross（amount_mop 喺 prep 先 netoff 做 0），令 調整前/後 會含返嗰 gross → prep_tableau 用
        # 「amount_mop≈0 且 調整≈0 → 三維度歸 0」收返（見 prep 調整前/後 block）。
