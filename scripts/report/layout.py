...
# ── 續版＝兩欄文字（唔係全闊單欄）─────────────────────────────────────
# 原報告 s13/s16/s23 實測（--fmt）：
#   s13  x0.53 w4.78 │ x5.63 w4.66      s16  x0.55 w4.77 │ x5.52 w4.77
#   s23  x0.53 w4.99 │ x5.63 w4.78
# 起始 y 1.65~1.76、底去到 6.98（比 CONTENT_BOTTOM 6.72 更低），而且【冇資料來源行】。
PROSE2_W = 4.80         # 每欄闊
PROSE2_X2 = 5.60        # 右欄起點
PROSE2_Y = 1.65         # 兩欄文字版起始 y
PROSE2_BOTTOM = 6.78    # 兩欄文字可以去到幾低
#   ⚠ 原報告 s13 個框去到 6.98，但佢頁腳喺 master 入面；我哋頁腳係 slide-level
#     shape（y=6.85），框去到 6.95 就會疊住版權行。實測我哋自己出嘅 s15/s16
#     確認咗 y1.65 h5.30 → 底 6.95。收到 6.78 留返空隙。

# 主要發現（事項描述框）左右比例 —— 原報告 s30 實測：左圖 x0.62 w3.49、右文 x4.15 w6.13。
# 我哋本來左 4.55／右 4.98，表霸咗位、敘述迫窄。
FIND_LEFT_W = 3.55      # 左邊表／圖
FIND_TEXT_X = 4.15      # 右邊敘述起點
...
def prose_numbered(box, items, *, size=SZ_BODY, gap=7, indent=0.24, title=None, tsize=SZ_BODY_HEAD):
    ...
    for no, head, body in items:
        ...
        setfont(rb, size, color=INK)       # 同上：跟 INK，唔好寫死
