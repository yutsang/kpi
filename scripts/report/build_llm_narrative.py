...
def _adj_prompt(adj_type, amt_wan, projects):
    ...
    for name, find, mgmt, b2, ruling in projects[:6]:
        ...
        lines.append(seg)
    ctx = "\n".join(lines)
    return (f"以下係一項『潛在調整事項』嘅底層資料。審查底稿表2 係最權威來源（含政府跨司工作組逐司裁決）。"
            f"請寫一段報告摘要（約130-220字），說明該調整類型、金額、主要涉及嘅投資項目同調減原因；"
            f"務必帶出跨司工作組／各司局（如社會文化司、旅遊局、文化局）嘅具體意見同 KPMG 最終立場"
            f"（例：某司認可／不認可、KPMG 維持調整），最後點出審查建議（通常為建議剔除／調減）。\n\n{ctx}")
