...
def facility_activity(df, bucket_label, split=None) -> pd.DataFrame:
    ...
    d["_key"] = list(zip(d["_scope"].eq(0), d["dicj code"].map(_norm)))
    ...
    if ac:
        _bt = sum(sum(v) for k, v in ac.items()
                  if k in set(zip(d["_scope"].eq(0), d["dicj code"].map(_norm))))
...
def overview_by_bucket(df, bucket, plan, category=None, split=None):
    ...
    if is_py:
        ...
        # 完成率同【設施建設／活動舉辦】兩對欄都係留喺最後俾下游文字邏輯用，
        # _overview_extra 出表前一齊 drop 走。★ 設施/活動本來冇留 → 1.3 第2版
        #   （原報告 s12「區分設施建設/活動舉辦的投資金額」）成版出唔到，一路冇人為意。
...
def finding_by_sub(df, adj_type):
    ...
    d = df.copy()
    d["_adj"] = d["調整一級"].map(CANON).fillna(d["調整一級"])
    d.loc[~d["_adj"].isin(ADJ7), "_adj"] = ADJ_POST
    d = d[d["_adj"] == adj_type]
    if d.empty:
        return pd.DataFrame()
    d["_chg"] = pd.to_numeric(d["調整_萬"], errors="coerce").fillna(0.0)
...
# ── from make_report ──
BUILD_STAMP = "base f0b93d6 · content abb23633 · bundled 2026-09-14 19:03"
...
def render_overview_pages(prs, crumb, headline, table_df, bullets, *, sec=0, table_name=None,
    ...
    if rest_all:
        cols = fit_prose(rest_all, PROSE2_W, col_h,
                           head_size=SZ_BODY_HEAD, body_size=SZ_BODY) or [rest_all]
    ...
    for pi, (tdf, left, right) in enumerate(pages):
        ...
        y0 = max(topx, PROSE2_Y)
        h = PROSE2_BOTTOM - y0
        prose_box(slide, MARGIN, y0, PROSE2_W, h, left,
                    head_size=SZ_BODY_HEAD, body_size=SZ_BODY, gap=7)
        if right:
            prose_box(slide, PROSE2_X2, y0, PROSE2_W, h, right,
                        head_size=SZ_BODY_HEAD, body_size=SZ_BODY, gap=7)
            ...
            if sh["kind"] == "shape":               # 流程圖方框：底色 + 框線，字另外疊上去
                ...
                if sh.get("text"):
                    put(slide, sh["x"], sh["y"], sh["w"], sh["h"], sh["text"],
                          ...
                          color=_rgb(sh.get("color")) or INK, align=PP_ALIGN.CENTER)
                continue
            if sh["kind"] == "text":
                put(slide, sh["x"], sh["y"], sh["w"], sh["h"], sh["text"],
                      size=sh.get("size") or SZ_BODY, bold=sh.get("bold", False),
...
def _b2_adj_by_canon(by_adj):
    ...
    唔可以直接查 CANON：表2 寫「超出可計入範圍的内部資源支出」（簡體「内」），
    CANON 收咗「超過…内…」同「超出…內…」，就係差呢一個 combination。引號都
    有全形／半形之分。所以先正規化再 exact，唔中先用相似度 —— 呢種係命名差異，
    唔係真係對唔上，門檻放 0.75。"""
    def _n(s):
        return re.sub(r"[\s“”\"'‘’「」]", "", str(s)).replace("内", "內").replace("儘", "盡")
    canon = {_n(t): t for t in ADJ_ALL}
    out = {}
    for nm, rec in (by_adj or {}).items():
        t = canon.get(_n(nm)) or CANON.get(nm)
        if not t:
...
def render_findings(prs, ent_up, df, narr, llm=None, b2=None, b2adj=None):
    ...
    for adj in ADJ_ALL:
        ...
        if _rest:
            _cols = fit_prose(_rest, PROSE2_W, PROSE2_BOTTOM - PROSE2_Y,
        ...
        for pi, (chunk, chunk_r) in enumerate(pages):
            ...
            if pi == 0:
                ...
                y0 = max(top + 0.04, PROSE2_Y)
                h2 = PROSE2_BOTTOM - y0
                prose_box(slide, MARGIN, y0, PROSE2_W, h2, chunk,
                            head_size=SZ_BODY_HEAD, body_size=SZ_BODY)
                if chunk_r:
                    prose_box(slide, PROSE2_X2, y0, PROSE2_W, h2, chunk_r,
                                head_size=SZ_BODY_HEAD, body_size=SZ_BODY)
...
def main():
    ...
    split = load_split(qingdan) if qingdan else {}        # 清單逐年 capex/opex 拆分 + 當年計劃項目名單
    ...
    ov = overview_by_bucket(sdf, "2025年度投資計劃", plan, cat, split)
    ...
    for bk in ["2024年度計劃期後投資", "2023年度計劃期後投資"]:
        ov = overview_by_bucket(sdf, bk, plan, cat, split)
        if not ov.empty:
    ...
    _d3 = sdf.copy()
    _d3["_adj"] = _d3["調整一級"].map(CANON).fillna(_d3["調整一級"])
    _d3.loc[~_d3["_adj"].isin(ADJ7), "_adj"] = ADJ_POST
    _have = [t for t in ADJ_ALL
    ...
    if narr:      # 逐調整類型 × 項目：金額(feed) + 事項描述(表2 原文) / 項目層抄清單
        ...
        try:            # 表2＝審查底稿，清單冇料時頂住（加密檔，開唔到就靜靜跳過）
            b2 = load_biao2_struct(_b2dir, entity, log=lambda *a: None)
        except Exception:
            pass
        try:            # 類型層：主要發現嗰段事項描述嘅【原文】就喺表2，唔使 LLM 重寫
            b2adj = _b2_adj_by_canon(load_biao2_by_adj(_b2dir, entity, log=lambda *a: None))
            if b2adj:
                print(f"  · 表2 事項描述原文：{len(b2adj)}/{len(ADJ_ALL)} 個調整類型")
        except Exception as e:
    ...
    for bk in BUCKET_ORDER:
        fa = facility_activity(sdf, bk, split)
        if not fa.empty:
