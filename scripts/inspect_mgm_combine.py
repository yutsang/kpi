...
def main():
    ...
    # prefer a real amount column by NAME (avoid Identifier/Journal/Period IDs that have huge sums)
    AMT_SKIP = re.compile(r"identifier|journal|\bref\b|number|period|book|hierarchy|level|section no|\bdate\b|^company$|^project", re.I)
    amt_col = next((c for c in df.columns
                    if ("debit minus credit" in str(c).lower() or "金額" in str(c) or "amount" in str(c).lower())
                    and "transaction" not in str(c).lower()), None)
    if not amt_col:
        best = -1.0
        for c in df.columns:
            if AMT_SKIP.search(str(c)):
                continue
            s = numify(df[c])
            if s.notna().mean() > 0.5 and s.abs().sum() > best:
                amt_col, best = c, s.abs().sum()
    ...
    # value samples for the NG/scope candidate columns — so we know which one holds the NG中文
    print("\n=== value samples (which col = NG中文 / Gaming-scope?) ===")
    for c in [x for x in df.columns if x in ("Section", "Section.1", "Section No.", "Nature", "Category", "Source")]:
        vc = df[c].astype(str).str.strip().replace("nan", "(blank)").value_counts().head(8)
        nonblank = (df[c].astype(str).str.strip().replace("nan", "").ne("")).mean() * 100
        print(f"  {c!r:14s} fill={nonblank:3.0f}%  top: " + ", ".join(f"{v!r}×{n}" for v, n in vc.items()))
